...
void Tutorial03_Texturing::Update(double CurrTime, double ElapsedTime)
{
    ...
    // Matriz de escalado no uniforme: (X=2.5, Y=0.1, Z=1.0)
    auto Scale = float4x4::Scale(2.5f, 0.1f, 1.0f);

    // Rotaciones originales
    auto RotY = float4x4::RotationY(static_cast<float>(CurrTime) * 1.0f);
    auto RotX = float4x4::RotationX(-PI_F * 0.1f);

    // Cámara y proyección 
    float4x4 View        = float4x4::Translation(0.f, 0.0f, 5.0f);
    auto     SrfPreTrans = GetSurfacePretransformMatrix(float3{0, 0, 1});
    auto     Proj        = GetAdjustedProjectionMatrix(PI_F / 4.0f, 0.1f, 100.f);

    // Ahora el modelo lleva primero escalado, luego rotaciones
    m_WorldViewProjMatrix = Scale * RotY * RotX * View * SrfPreTrans * Proj;
}
